...
export const LocationSamples = () => {
  ...
  return (
    ...
    <>
      <input className="form-control in" placeholder="Search by name" type="text" onChange={(i) => setSearch(i.target.value)} style={{ maxWidth: "60%" }} />
          ...
          >
            {
              ...
              muestras.filter((i) => {
                return (
                  search === "" ? i : i.user_lastname.toLocaleLowerCase().includes(search) || i.user_name.toLocaleLowerCase().includes(search)
                )
              }).map((muestra) => (
                <Marker
                  key={muestra.id}
